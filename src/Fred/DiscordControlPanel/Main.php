...
class Main extends PluginBase implements Listener
{
    ...
    public function onEnable(): void
    {
        ...
        $discord = $this->getServer()
            ->getPluginManager()
            ->getPlugin("DiscordBot");
        if (!$discord instanceof DiscordBot) {
            throw new PluginException(
                "Incompatible dependency 'DiscordBot' detected, see https://github.com/DiscordBot-PMMP/DiscordBot/releases for the correct plugin."
            );
        }
        ...
        $this->config = new Config(
            ...
            [
                ...
                "CommandBan" => "!ban",
                "CommandBanAllowedChannel" => [
                    115678910111213,
                    1063456789101112,
                ],
                "CommandUnban" => "!unban",
                "CommandUnbanAllowedChannel" => [
                    115678910111213,
                    1063456789101112,
                ],
                "CommandKick" => "!kick",
                "CommandKickAllowedChannel" => [
                    115678910111213,
                    1063456789101112,
                ],
                "CommandSay" => "!say",
                "CommandSayAllowedChannel" => [
                    115678910111213,
                    1063456789101112,
                ],
            ]
    ...
    public function MessageSent(MessageSent $event)
    {
        ...
        if ($server_id == 1234567890) {
            $this->getLogger()->info(
                "§cPlease add discord server id in config.yml"
            );
            $this->getServer()
                ->getPluginManager()
                ->disablePlugin($this);
        ...
        $isCommandAllowed = function (
            string $command,
            string $channel_id
        ): bool {
            $allowed_channels = $this->config->get(
        ...
        $getAllowedChannelsString = function (string $command): string {
            ...
            return implode(
                ", ",
                array_map(fn($id) => "<#$id>", $allowed_channels)
            );
        };
        ...
        if ($args[0] == $this->config->get("CommandList", "!list")) {
            if ($isCommandAllowed("CommandList", $channel_id)) {
                ...
                $players = implode(
                    "\n",
                    array_map(
                        fn(Player $player) => $player->getName(),
                        $onlines
                    )
                );
                $api->sendMessage(
                    ...
                    [
                        new Embed(
                            ...
                            count($onlines) === 0
                                ? []
                                : [new Field("Players", $players, true)]
                        ),
                    ]
                ...
                $api->sendMessage(
                    $server_id,
                    $channel_id,
                    "Commands `!list` can only be executed on channels " .
                        $getAllowedChannelsString("CommandList")
                );
            }
        }
            ...
            if ($isCommandAllowed("CommandBan", $channel_id)) {
                if (isset($args[1])) {
                    ...
                    if ($player instanceof Player) {
                        $player->kick(
                            "You have been banned by a Discord command."
                        );
                    }
                    $api->sendMessage(
                        $server_id,
                        $channel_id,
                        "Player $playerName has been banned and added to banned-players.txt."
                    );
                } else {
                    $api->sendMessage(
                        $server_id,
                ...
                $api->sendMessage(
                    $server_id,
                    $channel_id,
                    "Commands `!ban` can only be executed on channels " .
                        $getAllowedChannelsString("CommandBan")
                );
            }
        }
        ...
        if ($args[0] == $this->config->get("CommandUnban", "!unban")) {
            if ($isCommandAllowed("CommandUnban", $channel_id)) {
                if (isset($args[1])) {
                    ...
                    if ($this->isPlayerBanned($playerName)) {
                        ...
                        $api->sendMessage(
                            $server_id,
                            $channel_id,
                            "Player $playerName has been unbanned and removed from banned-players.txt."
                        );
                    } else {
                        $api->sendMessage(
                            $server_id,
                            $channel_id,
                            "Player $playerName is not banned."
                        );
                    }
                } else {
                    $api->sendMessage(
                ...
                $api->sendMessage(
                    $server_id,
                    $channel_id,
                    "Commands `!unban` can only be executed on channels " .
                        $getAllowedChannelsString("CommandUnban")
                );
            }
        }
        ...
        if ($args[0] == $this->config->get("CommandKick", "!kick")) {
            if ($isCommandAllowed("CommandKick", $channel_id)) {
                if (isset($args[1])) {
                    ...
                    if ($player instanceof Player) {
                        $player->kick(
                            "You have been kicked by a Discord command."
                        );
                        $api->sendMessage(
                            $server_id,
                            $channel_id,
                            "Player $playerName has been kicked."
                        );
                    } else {
                        $api->sendMessage(
                            $server_id,
                ...
                $api->sendMessage(
                    $server_id,
                    $channel_id,
                    "Commands `!kick` can only be executed on channels " .
                        $getAllowedChannelsString("CommandKick")
                );
            }
        }
        ...
        if ($args[0] == $this->config->get("CommandSay", "!say")) {
            if ($isCommandAllowed("CommandSay", $channel_id)) {
                ...
                $api->sendMessage(
                    $server_id,
                    $channel_id,
                    "Message broadcasted: \"$messageContent\""
                );
            } else {
                $api->sendMessage(
                    $server_id,
                    $channel_id,
                    "Commands `!say` can only be executed on channels " .
                        $getAllowedChannelsString("CommandSay")
                );
            }
        }
    }
